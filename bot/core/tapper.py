...
def convert_to_hmac(t=None, e=""):
    ...
    if e == "":
        e = ""
    # Sort the dictionary by keys and create a query string
    sorted_items = sorted(t.items())
    query_string = "&".join(f"{key}={value}" for key, value in sorted_items)
    # Perform HMAC SHA256 and return the hex digest
...
class Tapper:
    ...
    async def complete_task(self, task, session: cloudscraper.CloudScraper):
        ...
        try:
            ...
            res.raise_for_status()
            if res.json()["retCode"] == 0:
                logger.success(f"{self.session_name} | <green>Successfully completed task: <cyan>{task['taskName']}</cyan> - Earned: <cyan>{task['point']}</cyan></green>")
            else:
    ...
    async def claim_farm(self, session: cloudscraper.CloudScraper):
        try:
            ...
            res.raise_for_status()
            if res.json()["retCode"] == 0:
                logger.success(f"{self.session_name} | <green>Successfully claimed <cyan>{round(self.speed*self.time, 1)}</cyan> points from farm</green>")
            else:
    ...
    async def post_score(self, start_time, sign, points, end_time, session: cloudscraper.CloudScraper):
        ...
        try:
            ...
            res.raise_for_status()
            if res.json()["retCode"] == 0:
                logger.success(f"{self.session_name} | <green>Successfully earned <cyan>{points}</cyan> points from game!</green>")
                return True
            else:
                ...
                if can_run:
                    ...
                    if login_data:
                        ...
                        if settings.AUTO_GAME is True:
                            ...
                            logger.info(f"{self.session_name} | Ticket left: {game_stats['totalCount']-game_stats['usedCount']}/{game_stats['totalCount']}")
                            if game_stats['usedCount'] == game_stats['totalCount']:
                                ...
                                while totalTickets > 0:
                                    ...
                                    logger.info(f"{self.session_name} | Wait {round(sleep_, 2)} seconds to complete game...")
